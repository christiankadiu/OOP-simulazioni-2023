...
public class Test {
	...
	/*
	 * Implementare l'interfaccia EitherFactory come indicato nel metodo initFactory
	 * qui sotto.
	 * Realizza una factory per dei Either<A,B> ("A oppure B"), ossia delle varianti
	 * di Optional dove un valore
	 * (di tipo B) può essere presente (caso di "successo"), ma se non c'è (caso di
	 * "fallimento")
	 * allora è presente un valore che rappresenta il motivo del fallimento (di tipo
	 * A).
	 * Quindi dentro un Either o è presente il valore di successo, o è presente
	 ...
	 * (ossia, nella parte obbligatoria è sufficiente implementarne 2 di questi 3,
	 * il terzo è quindi opzionale)
	 * 
	 * - la buona progettazione della soluzione, riusando più codice possibile e
	 * tentendo tutte le implementazioni
	 * molto succinte
	 * 
	 ...
	 * 
	 * - qualità della soluzione: 5 punti (per la buona progettazione della
	 * soluzione, come sopra indicato)
	 * 
	 */
	...
	public void testException() {
		// il risultato di 0/0 è un failure, contenente l'eccezione lanciata, ossia
		// ArithmeticException
	...
	public void testFlatMap() {
		// ottenuto un risultato x, convertiamo in 10/x, ma funziona solo se era un
		// successo e se 10/x si può fare...
	...
	public void testFilter() {
		...
		assertEquals(Optional.of("negative!"),
				this.factory.of(() -> 0 / 0).filterOrElse(x -> x > 0, "negative!").getFailure());
		assertEquals(Optional.of("negative!"),
				this.factory.of(() -> -5).filterOrElse(x -> x > 0, "negative!").getFailure());
		assertEquals(Optional.of(5), this.factory.of(() -> 5).filterOrElse(x -> x > 0, "negative!").getSuccess());
	}

	@org.junit.Test
	public void testFold() {
		// applica una funzione in caso di successo e una in caso di fallimento:
		// qualcosa torna quindi sempre...
	...
	public void testTraverseOK() {
		// applica il /2 a una lista di interi pari: il tutto in questo caso è un
		// successo!
		var list = List.of(10, 20, 30, 42);
		var out = this.factory.traverse(list,
				i -> this.factory.of(() -> i).filterOrElse(j -> j % 2 == 0, "dispari").map(j -> j / 2));
